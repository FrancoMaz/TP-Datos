...
def entrenamiento(setDeDatos):
	for i in range(0,len(setDeDatos)):
		...
		contadorDeControl = 0
		errorCometido =  error(salidasDeseadas[int(fila[0])],salidasCapaFinal)
...
cantidadDeEpocas = 1 
while cantidadDeEpocas <= 2 :
	entrenamiento(setDeDatos)
	if(cantidadDeEpocas > 1):	
		precisionActual = calcularPrecisionDelEntrenamiento()
		print "Epoca:"+str(cantidadDeEpocas)+" ---presicion: " + str(precisionActual)+"%"		
	cantidadDeEpocas += 1
